...
def run_simulation(params, cost_function_options, parameters_info, 
    ...
    # Save results
    np.savetxt(os.path.join(sim_path, "params.txt"), np.array(params))
...
def bayesian_optimization(parameters):
    ...
    sampler = qmc.LatinHypercube(d=len(parameters['bounds']))
    X_raw = sampler.random(n=parameters['lhs']['nsamples'])
        
    # Scale the samples to the parameter bounds
    bounds = np.array([parameters['bounds'][b] for b in parameters['bounds']])
    ...
    space = space = [Real(parameters['bounds'][b][0],parameters['bounds'][b][1], name=b) for b in parameters['bounds']]   # ensure K_d < K_cw later
    ...
    for r in range(exploring_rounds):
        ...
        if verbose:
            for e, x in enumerate(Xs_explore):
                print("   X[%i]:"%e+" ("+(" %.5f,"*len(x)%tuple([x_ for x_ in x]))[:-1]+")" )
