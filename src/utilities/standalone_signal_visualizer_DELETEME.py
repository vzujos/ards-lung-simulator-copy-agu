...
if True:
    ...
    roots = ["C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/Codes/DeleteMe/PIG%i/"%simulation+sim for sim in ["sim_105/",] ]
    roots += ["C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/ards-lung-simulator/PIG%i-%s-per-3/"%(simulation,mesh_type)]
    ...
    for root,color in zip(roots,colors):
        ...
        cm = 'per'
        renamer = {'bir':'[bir] Yeoh-type constitutive model',
                   'ma':"Exponential constitutive model",
                   'per':'[per] Yeoh-type constitutive model',}
        ...
        if include_signals:
            ...
            ones = np.ones_like(measured_vol)
            
            X = np.vstack([measured_vol,measured_flow,ones]).T
            Amid = np.linalg.inv(X.T@X)
            A = Amid@X.T@measured_Paw
        ...
        ones = np.ones_like(volumes)
        X = np.vstack([volumes,fluxes,ones]).T
        Amid = np.linalg.inv(X.T@X)
        A = Amid@X.T@pressures
        ...
        print(" > (PIG%i) Simulated %.1f | Experimental %.1f  | Error %.0f"%(simulation, C_rs_sim, C_rs_exp, err_crs))
        print("Resistance in cmH2O/L/s")
        print(" > (PIG%i) Simulated %.1f | Experimental %.1f  | Error %.0f"%(simulation, R_rs_sim, R_rs_exp, err_raw))
        print()
