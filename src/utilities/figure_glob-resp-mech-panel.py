...
def scatter_and_bland_altman(field, data_manager, key_exp, key_sim, figsize=(12, 4), 
    ...
    plt.show()
# %% Generate multiple subject database
...
# %% Generate matrix display for DGF GF and Jacobian (5x3) visualization.
# Not in paper
...
# %% Generate matrix visualization for aeration compartments subject-wise (5x4) for aeration compartmnets
...
# %% Generate a local database for compliance, resistance and basal pressure
...
# %% Deprecated maybe? Broken function

if False:

    for field in ['DGF','NAT','PAT','AT','HIT','EI-porosity']:
...
# %% Generates the blant altman and scatter plot for Crs, Rrs and P0
    # Deprecated since we decided to drop P0 

if False:
    rename = {'Raw':"Airway resistance\n(cmH$\mathbf{_2}$O/L/s)",
              'Crs':"Respiratory system\ncompliance (mL/cmH$\mathbf{_2}$O)",
    ...
    fig,axes = plt.subplots(ncols=3, nrows=2, figsize=(3.0*3,2.5*2),dpi=300)
    ...
    for field, ax_scatter, ax_ba in zip([ "Crs", "Raw","PEEP"],
                                        axes[1, :],   # bottom row: scatter
                                        axes[0, :]    # top row: Bland–Altman
                                        ):
        ...
        ax_ba.scatter(mean_values, diff_values, marker='x', color='royalblue')
        
        ax_ba.axhline(mean_diff, color='k', lw=1.2,
                      label=f'Mean bias = {mean_diff:.2f}')
        ax_ba.axhline(loa_upper, color='gray', ls='--', lw=1.0,)
        ax_ba.axhline(loa_lower, color='gray', ls='--', lw=1.0,)
        
        # Symmetric y-limits
        y0, y1 = ax_ba.get_ylim()
        ymax = np.max(np.abs([y0, y1]))
        ax_ba.set_ylim(-ymax, ymax)
        
        # Enlarge ylims for compliance so the line is better visible
        if field == 'Crs':
            ax_ba.set_ylim(-6.0, 6.0)
    
        
        # Format axis
        ax_ba.set_xlabel('Mean of Experiment and Simulation', size=fs-1)
        ax_ba.set_ylabel('Simulation − Experiment', size=fs-1)
        ax_ba.set_title(rename[field],size=fs,weight='bold')
        ...
        ax_ba.set_xticks(ticks)
        
        if field == 'PEEP':
            ax_ba.set_yticks(np.linspace(-1,1,5))
        
        if field == 'Raw':
            ax_ba.legend(frameon=False, fontsize=fs-1, loc='lower left')
        elif field == 'Crs':
            ax_ba.legend(frameon=False, fontsize=fs-1, loc='lower left')
        else:
            ax_ba.legend(frameon=False, fontsize=fs-1, loc='lower left')
        ...
        # Scatter plot
        slope, intercept, r_value, p_value, _ = linregress(xdata, ydata)
        ax_scatter.scatter(xdata, ydata, marker='x', color='royalblue')
        x_fit = np.linspace(min(xdata), max(xdata), 100)
        y_fit = slope * x_fit + intercept
        ...
        ax_scatter.plot(x_fit, y_fit, color='k', lw=1.5,
                        label='r = %.2f ' % r_value + text)
        
        # Format axis
        ax_scatter.set_ylabel("Simulation", size=fs-1)
        ax_scatter.set_xlabel("Experiment", size=fs-1)
        ax_scatter.spines['top'].set_visible(False)
        ax_scatter.spines['right'].set_visible(False)
        ax_scatter.set_xlim(bounds[field])
        ax_scatter.set_ylim(bounds[field])
        ...
        # Format axis
        ax_scatter.set_title(rename[field],size=fs,weight='bold')
    
        ax_scatter.set_xticks(ticks)
        ax_scatter.set_yticks(ticks)
        x0,x1 = ax_scatter.get_xlim(); dx = x1-x0
        y0,y1 = ax_scatter.get_ylim(); dy = y1-y0
        ax_scatter.plot([x0,x1],[y0,y1], ls='--',alpha=0.3, color='k',label='Identity')
        ax_scatter.legend(fontsize=fs-1,frameon=False)
    ...
    plt.savefig('./figures/grm-panel.pdf',dpi=300, bbox_inches='tight')
...
if True:
    ...
    # Select the subject under analysis
    subject = 4
    ...
    case_path = sim_path+case

   # case_path =   "C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/Codes/DeleteMe/PIG%i/sim_035/"%subject
  ...
  #  t = signals['time']
    ...
    # Compute the values for the simulation
    E_rs = A[0]
    C_rs_exp = 1/E_rs*1000 # mL/cmH2O
    R_rs_exp = A[1]
    PEEP_exp = A[2]
    
    err_crs = np.abs(C_rs_exp - C_rs_sim)/C_rs_exp*100
    err_raw = np.abs(R_rs_exp - R_rs_sim)/R_rs_exp*100
    print("Compliance in mL/cmH2O")
    print(" > (PIG%i) Simulated %.1f | Experimental %.1f  | Error %.0f"%(subject, C_rs_sim, C_rs_exp, err_crs))
    print("Resistance in cmH2O/L/s")
    print(" > (PIG%i) Simulated %.1f | Experimental %.1f  | Error %.0f"%(subject, R_rs_sim, R_rs_exp, err_raw))
    
# %% Reduced respiratory mechanics panel
# Currently in paper
...
if True:
    ...
    fig,axes = plt.subplots(ncols=2, nrows=2, figsize=(3.0*2,2.5*2),dpi=300)
    ...
    for field, ax_scatter, ax_ba in zip([ "Crs", "Raw",],
                                        axes[:, 1],   # bottom row: scatter
                                        axes[:, 0]    # top row: Bland–Altman
                                        ):
        ...
        if field == 'PEEP':
            ...
            ax_ba.set_yticks(np.linspace(-8,8,5))
        elif field == 'Raw':
            ax_ba.set_yticks(np.linspace(-6,6,7))
