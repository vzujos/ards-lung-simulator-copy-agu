...
# This panel generates the definitive figure respiratoryu mechanics signals for the 'representative subject'
...
if True:
    ...
    root += "PIG%i-%s-per"%(simulation,subjmesh[simulation])

  #  root = "C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/Codes/DeleteMe/PIG%i/sim_035/"%simulation
    ...
    expcolor = 'k'#'royalblue'
    simcolor = 'royalblue'#'k'
    ls='--'
    ...
    # Plot situation
    axes[0].plot(times[:-1],-fluxes[:-1],alpha=1.0,color=simcolor,label="Simulation",lw=lw)
    xmin,xmax = axes[0].get_xlim()
    axes[0].axhline(0.0,xmin=xmin,xmax=xmax,color="silver",ls="--",lw=1.0, alpha=0.5)
    axes[0].set_xticks([])
    axes[0].set_ylabel("Air flow\n(L/s)")
    axes[0].set_ylim(-0.70,1.0)
    axes[0].axvspan(0.0, Tinsp,  color=simcolor, alpha=0.1)
    axes[0].fill_between(times[times<Tinsp],-fluxes[times<Tinsp], color=simcolor,alpha=0.5)

            
    axes[1].plot(times,volumes,alpha=1.0,color=simcolor,lw=lw)
    axes[1].axvspan(0.0, Tinsp,  color=simcolor, alpha=0.1)
    axes[1].fill_between(times[times<Tinsp],volumes[times<Tinsp], color=simcolor,alpha=0.5)
    axes[1].axhline(0.0,xmin=xmin,xmax=xmax,color="silver",ls="--",lw=1.0, alpha=0.5)
    axes[1].set_xticks([])
    axes[1].set_ylabel("Lung volume\n(L)")
        
    axes[2].plot(times,pressures,alpha=1.0,color=simcolor,lw=lw)
    axes[2].axhline(0.0,xmin=xmin,xmax=xmax,color='silver',ls="--",lw=1.0, alpha=0.25)
    axes[2].set_ylabel("Airway pressure\n(cmH$_{2}$O)")
    axes[2].set_xlabel("Time (s)")
    axes[2].set_ylim((0,30))
    ymin,ymax = axes[2].get_ylim()
    axes[2].axvspan(Tinsp, Tcycle, ymin=ymin, ymax=ymax, color=simcolor, alpha=0.1)
    axes[2].fill_between(times[times>Tinsp],pressures[times>Tinsp], color=simcolor,alpha=0.5)
    
    for ax in axes:
     #   ax.axvline(0.0,color="k",ls="--",alpha=0.5)
        ax.axvline(Tsyr,color="k",ls="-",alpha=0.2)
        ax.axvline(Tinsp,color="k",ls="-",alpha=0.2)
      #  ax.axvline(Tcycle,color="k",ls="--",alpha=0.5)
        ax.set_xlim((0,2.5))

    axes[2].set_xticks(np.linspace(0,2.5,6))
    if include_signals:
        ...
        col = 'k'
        axes[0].plot(measured_time,-measured_flow, alpha=1.0,color=expcolor,label="Experiment",lw=lw, ls=ls)
        axes[1].plot(measured_time,measured_vol , alpha=1.0,color=expcolor,lw=lw, ls=ls)
        axes[2].plot(measured_time,measured_Paw , alpha=1.0,color=expcolor,lw=lw,ls=ls)
    ...
    plt.savefig('./figures/grm-signals.pdf',dpi=300, bbox_inches='tight')

# %% Legacy block that evaluates error measures
...
# %% Processing error measurements for a boxplot



database = {'Coarse':{'costs':[],
                   'flux':{'MAE':[],
                           'RMSE':[],
                           'MAPE':[],
                           'NRMSE':[],
...
root_cal = "C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/Codes/DeleteMe/"
root_sim = "C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/ards-lung-simulator/"
...
paths = {"Coarse":{2:root_cal+"PIG2/sim_040/",
                   3:root_cal+"PIG3/sim_079/",
                   4:root_cal+"PIG4/sim_178/",
                   5:root_cal+"PIG5/sim_285/",
                   6:root_cal+"PIG6/sim_125/"},
         "Fine":{2:root_cal+"PIG2/sim_046/",
                 3:root_cal+"PIG3/sim_014/",
                 4:root_cal+"PIG4/sim_080/",
                 5:root_cal+"PIG5/sim_115/",
                 6:root_cal+"PIG6/sim_030/"}
         }


for mesh_type in paths:

    for subject in [2,3,4,5,6]:
        
        path_to_data = paths[mesh_type][subject]+"/Signals/"
        sim_path = paths[mesh_type][subject]
        ...
        database[mesh_type]['costs'] += [determine_misfits_from_signal(sim_path, calibration_config, wave_config,
        ...
        database[mesh_type]['flux']['MAE'] += [mae]
        database[mesh_type]['flux']['RMSE'] += [rmse]
        database[mesh_type]['flux']['MAPE'] += [mape]
        database[mesh_type]['flux']['NRMSE'] += [nrmse*100]
        ...
        database[mesh_type]['pressure']['MAE'] += [mae]
        database[mesh_type]['pressure']['RMSE'] += [rmse]
        database[mesh_type]['pressure']['MAPE'] += [mape]
        database[mesh_type]['pressure']['NRMSE'] += [nrmse*100]
        
# %% Boxplot visuazation for error signals
...
for e,(cm,col) in enumerate(zip(paths,colors)):
...
# Legend elements
legend_elements = [ Patch(facecolor='tab:blue',   edgecolor='none', label='Coarse'),
                   Patch(facecolor='tab:orange', edgecolor='none', label='Fine') ]
...
fig, axes = plt.subplots(nrows=3, figsize=(6,3),dpi=300)
lw=2.5
measured_vol = mat['volume'].flatten()
measured_flow = mat['flow'].flatten()
measured_Paw = mat['pressure'].flatten()
...
Paw = np.hstack([measured_Paw,measured_Paw,measured_Paw])
Vol = np.hstack([measured_vol,measured_vol,measured_vol])
Flow = np.hstack([measured_flow,measured_flow,measured_flow])
Time = np.hstack([measured_time,measured_time+np.max(measured_time),measured_time+2*np.max(measured_time)])

col = 'k'
axes[0].plot(Time,Paw , alpha=1.0,color=col,lw=lw)
axes[1].plot(Time,-Flow, alpha=1.0,color=col,label="Experiment",lw=lw)
axes[2].plot(Time,Vol , alpha=1.0,color=col,lw=lw)



for letter,ax in zip(["P","Q","V"],axes):
    ax.spines['top'].set_visible(False)
    ax.spines['left'].set_visible(False)
    ax.spines['right'].set_visible(False) 
    ax.spines['bottom'].set_visible(False)
   # ax.axhline(0.0, color='gray', alpha=.80, ls ='--')
    ax.set_xticks([])
    ax.set_yticks([])
    y0,y1 = ax.get_ylim()
    ax.text(-1.0,(y1+y0)/2,letter,size=32,weight='bold')
...
# Generate a figure for a larger scheme under development in Inkscape
...
if True:
    ...
    fig, axes = plt.subplots(nrows=2,ncols=1,figsize=(4,4),dpi=300)
    ...
    xmin,xmax = axes[0].get_xlim()
    axes[0].axhline(0.0,xmin=xmin,xmax=xmax,color="k",ls="--",lw=1.0, alpha=0.5)
    axes[0].set_xticks([])
    axes[0].set_ylabel("")
    axes[0].axvspan(0.0, Tinsp,  color='royalblue', alpha=0.1)
    axes[0].fill_between(times[times<Tinsp],-fluxes[times<Tinsp], color='royalblue',alpha=0.25)

    
        
    axes[1].plot(times,pressures,alpha=1.0,color=c,lw=lw)
    axes[1].axhline(0.0,xmin=xmin,xmax=xmax,color="k",ls="--",lw=1.0, alpha=0.5)
    axes[1].set_ylabel("")
    axes[1].set_xlabel("")
    ymin,ymax = axes[1].get_ylim()
    axes[1].axvspan(Tinsp, Tcycle, ymin=ymin, ymax=ymax, color='royalblue', alpha=0.1)
    axes[1].fill_between(times[times>Tinsp],pressures[times>Tinsp], color='royalblue',alpha=0.25)
    
    for letter,ax in zip(["Q","P"],axes):
        ax.axvline(Tsyr,color="k",ls="--",alpha=0.2)
        ax.axvline(Tinsp,color="k",ls="--",alpha=0.2)
        ax.set_xticks([])
        ax.set_yticks([])
        ...
        ax.spines['bottom'].set_visible(False)
        y0,y1 = ax.get_ylim()
        ax.text(-0.25,(y0+y1)/2.0,letter,size=32)
    ...
    if include_signals:
        ...
        col = 'k'
        axes[0].plot(measured_time,-measured_flow, alpha=0.8,color=col,label="Experiment",lw=lw)
        axes[1].plot(measured_time,measured_Paw , alpha=0.8,color=col,lw=lw)
