...
# Path to files
root_path = 'C:/Users/angus/OneDrive - Universidad Católica de Chile/Documentos/Codes/DeleteMe/'

cases = ['PIG2/sim_046','PIG3/sim_125','PIG4/sim_025','PIG5/sim_115','PIG6/sim_113',]
...
# %%
rename = {'Raw':"R$_{aw}$ (cmH$_2$O/L/s)",
          'Crs':"C$_{rs}$ (mL/cmH$_2$O)",
          'PEEP':"P$_{0}$ (cmH$_2$O)"}
...
fig,axes = plt.subplots(ncols=3, nrows=2, figsize=(9,6),dpi=300)
...
for field, ax_scatter, ax_ba in zip([ "Crs", "Raw","PEEP"],
    ...
    # Format axis
    ax_ba.set_xlabel('Mean (Exp + Sim) / 2', size=fs)
    ax_ba.set_ylabel('Sim − Exp', size=fs)
    ax_ba.set_title(rename[field],size=fs+2)
    ax_ba.spines['top'].set_visible(False)
    ax_ba.spines['right'].set_visible(False)
    
    if field == 'Raw':
        ax_ba.legend(frameon=False, fontsize=fs, loc='lower left')
    elif field == 'Crs':
        ax_ba.legend(frameon=False, fontsize=fs, loc='lower left')
    else:
        ax_ba.legend(frameon=False, fontsize=fs, loc='lower right')
    ...
    # Format axis
    ax_scatter.set_ylabel("Simulation", size=fs)
    ax_scatter.set_xlabel("Experiment", size=fs)
    ...
    # Format axis
    ax_scatter.set_title(rename[field],size=10)

    ax_scatter.set_xticks(ticks)
    ax_scatter.set_yticks(ticks)
    ax_scatter.legend(fontsize=fs)
    x0,x1 = ax_scatter.get_xlim(); dx = x1-x0
    y0,y1 = ax_scatter.get_ylim(); dy = y1-y0
    ax_scatter.plot([x0,x1],[y0,y1], ls='--',alpha=0.3, color='k')
    
    for s,subj in enumerate(subjects):
        ax_scatter.text(xdata[s]+dx*0.075,ydata[s],"%i"%subj,ha='center', va='center')
...
for ax,txt in zip(axes.flatten(),['(a)','(b)','(c)','(d)','(e)','(f)']):
    ...
    ax.text(x0-0.12*dx,y1+0.07*dy,txt,size=12,weight='bold')
...
plt.savefig('./figures/grm-panel.tiff',dpi=300, bbox_inches='tight')