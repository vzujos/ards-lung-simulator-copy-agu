...
simulations = ["PIG5-preload-nograv","PIG5-preload-grav"] # ,"gravity-tests/birzle-no-grav"
...
data_manager = {kind:{field:{'mean':[],'std':[]} for field in ['VS','Jacobian','DGF','NAT','PAT','AT','HIT',
                                                               ...
                                                               for kind in ['Birzle','Exponential','reg']}

data_manager.update({'pos':[]})
for kind in ['Birzle','Exponential']:
...
for subject in subjects:
    ...
    sim_signals_birzle = root_path + simulations[0]+"/Signals/"
    sim_signals_exp = root_path + simulations[1]+"/Signals/"
    ...
    sim_mesh_birzle = root_path + simulations[0]+'/post/full_0.750000000000.vtu'
    sim_mesh_exponential = root_path + simulations[1]+'/post/full_0.750000000000.vtu'

    
    # Load both meshes
    #rmesh = io.read(reg_mesh)
    birzle_mesh = io.read(sim_mesh_birzle)
    exp_mesh = io.read(sim_mesh_exponential)
    mass = np.ones_like(birzle_mesh.point_data['Pressure'])
    
    # Analyze regionally
    _, ids = IsoVolumetricSegmentation(directions[direction],mass,birzle_mesh.points, nrois)
    
    for mesh,kind in zip([exp_mesh,birzle_mesh],['Exponential','Birzle']):
        
        if kind == 'reg':
            j = mesh.point_data['Jacobian']
        elif kind in ['Exponential','Birzle']:
    ...
    for kind,path in zip(['Exponential','Birzle'],[sim_mesh_exponential,sim_mesh_birzle]):
...
# %%
    ...
    if True:            
        ...
        for kind, sim_signals in zip(['Exponential','Birzle'],[sim_signals_exp, sim_signals_birzle]):
...
rxdata = np.array(data_manager['Exponential']['EI-porosity']['mean'])
rydata = np.array(data_manager['Exponential']['Jacobian']['mean'])
sxdata = np.array(data_manager['Birzle']['EI-porosity']['mean'])
sydata = np.array(data_manager['Birzle']['Jacobian']['mean'])
...
rydata = np.array(data_manager['Exponential']['EI-porosity']['mean'])
sydata = np.array(data_manager['Birzle']['EI-porosity']['mean'])
...
ax.legend()
ax.text(0,0.15,'Ventral')
ax.text(8.0,0.15,'Dorsal')
#plt.show()
...
for (xdata,ydata),title in zip([(rydata,xdata),(sydata,xdata)],['No-Grav','Grav']):
    ...
    ax.plot(xdata,ydata,alpha=0.50, label=title,lw=2.5)
   # ax.set_title(title)
...
ax.set_xticks(np.linspace(0.9,1.4,6))
ax.text(0.74,0.0,'Ventral',weight='bold')
ax.text(0.74,9.3,'Dorsal',weight='bold')
ax.invert_yaxis()
ax.legend()
plt.tight_layout()
...
xdata = np.array(data_manager['pos'])
rydata = np.array(data_manager['Exponential']['DGF']['mean'])
sydata = np.array(data_manager['Birzle']['DGF']['mean'])
...
ax.set_xticks(np.linspace(0.0,0.15,6))
ax.text(-0.05,0.0,'Ventral',weight='bold')
ax.text(-0.05,8.7,'Dorsal',weight='bold')
...
for (xdata,ydata),title in zip([(xdata,rydata),(xdata,sydata)],['Exponential','Birzle']):
    ...
    ax.legend()
    ax.set_xlabel('ROI ID')
    ax.set_xticks(np.arange(nrois))
    ax.set_xticklabels(["%i"%(f+1) for f in np.arange(nrois)])
    
ax.set_ylabel('End-Inspiratory Porosity (-)')
ax.set_yticks(np.linspace(0.0,1.0,6))
ax.text(0.0,-0.1,'Ventral',weight='bold')
ax.text(8.3,-0.1,'Dorsal',weight='bold')
...
for ax, field in zip(axes, ['NAT','PAT','AT','HIT']):
    ...
    rydata = np.array(data_manager['Exponential'][field]['mean'])
    sydata = np.array(data_manager['Birzle'][field]['mean'])

    for (xdata,ydata),title in zip([(xdata,rydata),(xdata,sydata)],['Exponential','Birzle']):
      
       
        ax.plot(xdata,ydata,alpha=0.50, label=title)
   ...
   #     ax.legend()
    ...
    ax.set_ylabel('End-Inspiratory Porosity (-)')
    ax.set_yticks(np.linspace(0.0,1.0,6))
    ax.text(0.0,-0.18,'Ventral',weight='bold')
    ax.text(7.5,-0.18,'Dorsal',weight='bold')
