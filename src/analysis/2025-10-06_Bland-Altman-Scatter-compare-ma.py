...
def scatter_and_bland_altman(field, data_manager, kind,figsize=(12, 4), dpi=200,
    ...
    ydata = np.array(data_manager[kind][field]['mean'])  # Simulation values
...
def scatter_and_bland_altman_global(field, data_manager,kind,tags, figsize=(12, 4), dpi=200,
                             colors=['tab:blue', 'tab:orange', 'tab:green'],
...
data_manager = {kind:{field:{'mean':[],'std':[]} for field in ['VS','Jacobian','DGF','NAT','PAT','AT','HIT',
                                                               'PEEP','Crs','Raw','EE-porosity','EI-porosity']} for kind in ['sim-bir','sim-ma','reg']}
data_manager.update({'pos':[]})
for kind in ['sim-bir','sim-ma','reg']:
...
for subject in subjects:
    ...
    sim_signals_bir = simulated_path+organizer['sim-bir'][subject]+"/output/Signals/"
    sim_signals_ma = simulated_path+organizer['sim-ma'][subject]+"/output/Signals/"

    # Path to the time data
    sim_time_npy = sim_signals_bir + 'effectivetimes.npy'
    ...
    sim_mesh_bir = mesh_path+'sim_anim_ready - birzle.vtu'
    sim_mesh_ma = mesh_path+'sim_anim_ready - ma.vtu'

    
    # Load both meshes
    rmesh = io.read(reg_mesh)
    sbmesh = io.read(sim_mesh_bir)
    smmesh = io.read(sim_mesh_ma)
    ...
    for mesh,kind in zip([sbmesh,smmesh,rmesh],['sim-bir','sim-ma','reg']):
        
        if kind == 'reg':
            j = mesh.point_data['Jacobian']
        elif kind in ['sim-bir','sim-ma']:
    ...
    for kind,path in zip(['reg','sim-bir','sim-ma'],[reg_mesh, sim_mesh_bir,sim_mesh_ma]):
    ...
    for kind, sim_signals in zip(['sim-bir','sim-ma'],[sim_signals_bir, sim_signals_ma]):
...
scatter_and_bland_altman('Jacobian',data_manager,'sim-bir',dpi=300)
scatter_and_bland_altman('Jacobian',data_manager,'sim-ma',dpi=300)
...
bir_gf_data = np.array(data_manager['sim-bir']['DGF']['mean'])
bir_vs_data = np.array(data_manager['sim-bir']['VS']['mean'])
ma_gf_data = np.array(data_manager['sim-ma']['DGF']['mean'])
ma_vs_data = np.array(data_manager['sim-ma']['VS']['mean'])


fig,axes = plt.subplots(ncols=2,figsize=(8,4),dpi=300)

for ax,(xdata,ydata),title in zip(axes,[(bir_vs_data,ma_vs_data),(bir_gf_data,ma_gf_data)],['Volumetric Strain','Delta Gas Fraction']):
  ...
  #  ax.set_xlim((0,40))
   # ax.set_ylim((-0.025,0.125)) # dgf
    ax.legend()
    ax.set_xlabel("Birzle's model")
    ax.set_ylabel("Ma's model")
    
    if title == 'Volumetric Strain':
        ax.set_xlim((10,25))
        ax.set_ylim((10,25))
        ax.set_xticks([10,15,20,25])
        ax.set_yticks([10,15,20,25])
    else:
        ax.set_xlim((0.020, 0.050))
        ax.set_ylim((0.020,0.050))
