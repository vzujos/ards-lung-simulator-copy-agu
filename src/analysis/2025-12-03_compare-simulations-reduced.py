...
cases = ['PIG2-mf-bir','PIG5-mf-mediastinum','PIG6-mf-bir',]
...
for subj,case in zip(subjects,cases):
    ...
    # Extract data
    dm = extract_roi_data(subj, 'medium-fine', root_path+case,  tag="", nrois=10)
...
fig, axes = plt.subplots(ncols=3,nrows=3,figsize=(12,9),dpi=300)
...
for e,field in enumerate(['EI-porosity','Jacobian', 'DGF']):
    ...
    for s,subj in enumerate(subjects):
        ...
        xdata = np.array(dm['Sim: PIG%i/medium-fine'%subj][field]['mean'])
      #  xdata2 = np.array(dm2['Sim: PIG5/medium-fine'][field]['mean'])
        rxdata = np.array(dm['Exp: medium-fine'][field]['mean'])
...
for subj in [2,5,6]:
    ...
    for field in fields:
        ...
        gdm['Sim'][field]['mean'] += dms[subj]['Sim: PIG%i/medium-fine'%subj][field]['mean']
        gdm['Exp'][field]['mean'] += dms[subj]['Exp: medium-fine'][field]['mean']

    gdm['pos'] += dms[subj]['pos']

# %%

for field in ['Jacobian','DGF','NAT','PAT','AT','HIT','EI-porosity',"Crs","Raw","PEEP"]:
...
for field,ax in zip(["Raw", "Crs","PEEP"],axes):
    ...
    for subj in subjs:
        ...
        xdata += list(dms[subj]['Exp: medium-fine'][field]['mean'])
        ydata += list(dms[subj]['Sim: PIG%i/medium-fine'%subj][field]['mean'])
